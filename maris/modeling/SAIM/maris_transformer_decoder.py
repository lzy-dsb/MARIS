...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.depcls_proj = MLP(hidden_dim, hidden_dim, clip_embedding_dim, 3)
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret["clip_embedding_dim"] = cfg.MODEL.MARIS.EMBED_DIM
        return ret

    def forward(self, x, mask_features, cls_token, mask = None, text_classifier=None, num_templates=None):
        ...
        outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, cls_token, attn_mask_target_size=size_list[0],
        ...
        for i in range(self.num_layers):
            ...
            output = self.transformer_cross_attention_layers[i](
                output, 
                src[level_index], # multi-scale information with depth enhancement
            ...
            ####################### SPIM
            outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(output, mask_features, cls_token, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels],
                                                                                   text_classifier=text_classifier, num_templates=num_templates)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
    ...
    def forward_prediction_heads(self, output, mask_features, cls_token, attn_mask_target_size, text_classifier, num_templates):
        decoder_output = self.decoder_norm(output)
        decoder_output = decoder_output.transpose(0, 1)
        mask_embed = self.mask_embed(decoder_output)

        # mask_features from pixel decoder to interact with query embeddings
        mask_features = F.interpolate(cls_token, size=(mask_features.shape[-2], mask_features.shape[-1]), mode="bilinear", align_corners=False) + mask_features
        ...
        # text query
        cls_token = F.adaptive_max_pool2d(cls_token, (1, 1)).squeeze(-1).squeeze(-1).unsqueeze(1).expand(-1, self.num_queries, -1)
        cls_token = self.depcls_proj(cls_token)
        class_embed = class_embed + cls_token
